...
//		List<NoteDto> list = noteBiz.selectList(M_no);
//
...
//		}
	...
	//캘린더페이지로 이동
		...
		//캘린더에 띄울 
		@RequestMapping(value = "/getCalList.do")
		@ResponseBody
		public List<Map<String, String>> getCalList(HttpSession session) {
			MemberDto dto = (MemberDto) session.getAttribute("login");
			String m_id = dto.getM_name();
			List<NoteDto> list = new ArrayList<NoteDto>();
			Map<String, String> res = new HashMap<String, String>();
			List<Map<String, String>> result = new ArrayList<Map<String,String>>();
			try {
				list = noteBiz.calList(m_id);
			} catch (Exception e) {
				System.out.println("아이디값 안들어감");
			}
				for (int i = 0; i < list.size(); i++) {
					res.put("id", list.get(i).getNwriter());
					res.put("title", list.get(i).getNtitle());
					res.put("date", list.get(i).getNtoday());
					result.add(res);
					System.out.println(result);
					
				/*
				 * res.put("id", list.get(i).getNwriter()); res.put("title",
				 * list.get(i).getNtitle()); res.put("date", list.get(i).getNtoday());
				 */
				}
			return result;
		}
